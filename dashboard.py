# Real Estate Call Dashboard - Safe Google Sheet Sync Version
import pandas as pd
...
@st.cache_data
def load_data():
    sheet_url = "https://docs.google.com/spreadsheets/d/1BbYI691b7KGl_qGEBxL8WJCIc8ZGAQ6Rd8eucm72uKk/export?format=csv"
    ...
    df['Connection Rate'] = df['Conversations'] / df['Total Dials'].replace({0: None})
    ...
    df['Acceptance Rate'] = df['OFFER SIGNED'] / df['Offer Made'].replace({0: None})
    df['Contract Rate'] = df['CONTRACT SIGNED'] / df['Offer Made'].replace({0: None})
    df['Success Rate'] = df['CONTRACT SIGNED'] / df['Total Dials'].replace({0: None})
    return df
